...
| R-11 | UC-007 Desktop pane | Settings → Plugins → Rescan (or restart); open the `quota` tab in the right dock; toggle All/This profile; press ↻ | pane lists profiles with progress bars and alerts; numbers equal `/quota`; no chat session required | partial 2026-09-20: plugin listed as `Account Usage - on disk` and enabled in Settings > Plugins (loaded from the ROOT home `desktop-plugins/`); pane content not yet shown by the owner |
