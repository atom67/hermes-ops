...
| R-07 | UC-004 slash in Desktop | Desktop, profile mastermind: `/quota`, then `/quota local`, then `/quota daria` | multi-profile report with alert header; local = one profile; no model turn in the trace | not yet run by the owner (needs Desktop restart) |
