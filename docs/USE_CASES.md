...
|---|---|---|---|---|
| SET-001 | Plugin installed and enabled: `python install.py` (every home; `--profile <name>` for one) copies `plugin/account-usage` to `<HERMES_HOME>/plugins/` and adds `account-usage` to `plugins.enabled`; restart the profile's gateway/TUI/Desktop | project root / profile `config.yaml` | required | UC-001, UC-002, UC-003, UC-004, UC-005, UC-006 |
